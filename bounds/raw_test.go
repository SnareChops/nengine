...
func TestBoundsIsWithin(t *testing.T) {
	bounds := new(bounds.Raw).Init(4, 8)
	assert.False(t, bounds.IsWithin(-1, 4))
	assert.False(t, bounds.IsWithin(2, -1))
	assert.False(t, bounds.IsWithin(4, 4))
	assert.False(t, bounds.IsWithin(2, 8))

	assert.True(t, bounds.IsWithin(2, 0))
	assert.True(t, bounds.IsWithin(0, 4))
	assert.True(t, bounds.IsWithin(3, 4))
	assert.True(t, bounds.IsWithin(2, 7))
}
...
func TestRawResize(t *testing.T) {
	...
	x, y = b.Mid()
	assert.Equal(t, 99.5, x)
	assert.Equal(t, 99.5, y)
	x, y = b.Max()
